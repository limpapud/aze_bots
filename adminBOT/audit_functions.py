...
def check_role(message):
    if check_authentification(message) == 101:  # if authentication will complete with 101(Successful) code then
        conn = sqlite3.connect(config.adminBOTDB)  # connecting to adminBOTDB database.
        c = conn.cursor()  # defining connection cursor
        commanda = message.text  # assigning user message to variable
...
def check_password(message):
    try:
        ...
        hash = hashlib.md5(key_string).hexdigest()  # hashing password to MD5
